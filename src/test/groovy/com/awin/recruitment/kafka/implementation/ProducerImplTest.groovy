...
class ProducerImplTest extends Specification {

    final EXIT_MESSAGE =1

    def "test-transactions-generated-correctly"() {
        setup: "create consumer object"
        def queue=generateMessages()
        def producer=new ProducerImpl(queue)

        when: ""
        producer.run()

        then:
        assert producer.getOutput().size()==generateMessages().size()-EXIT_MESSAGE
    }

    BlockingQueue<Transaction> generateMessages() {
